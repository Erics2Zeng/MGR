...
public class CodeGenerator {
    ...
    private static  String PACKAGE_PATH_SERVICE = packageConvertPath(ProjectConstant.SERVICE_PACKAGE);//生成的Service存放路径
    private static  String PACKAGE_PATH_SERVICE_IMPL = packageConvertPath(ProjectConstant.SERVICE_IMPL_PACKAGE);//生成的Service实现存放路径
    private static  String PACKAGE_PATH_CONTROLLER = packageConvertPath(ProjectConstant.CONTROLLER_PACKAGE);//生成的Controller存放路径
    public static final String PACKAGE_PATH_COMMON_ENTITY = packageConvertPath(ProjectConstant.COMMON_ENTITY_PACKAGE);//生成common.entity路径
    ...
    public static void main(String[] args) {
        ...
        JdbcTemplate template = getJdbcTemplateInstanse();
        List<Map<String, Object>> maps = template.queryForList("select table_name from information_schema.tables where table_schema='access_gate'");
        List<String> table_names = maps.stream().map(map -> (String) map.get("TABLE_NAME")).collect(Collectors.toList());
        String[] tables = table_names.toArray(new String[table_names.size()]);
        genCode(tables);
    }

    private static JdbcTemplate getJdbcTemplateInstanse(){
        MysqlDataSource dataSource = new MysqlDataSource();
        dataSource.setURL(JDBC_URL);
        dataSource.setUser(JDBC_USERNAME);
        dataSource.setPassword(JDBC_PASSWORD);
//        (JDBC_DIVER_CLASS_NAME);
        //防止不同schema 下表名稱相同的情況，會查詢所有名稱相同的table
...
//        caseSensitive默认false，当数据库表名区分大小写时，可以将该属性设置为true
        pluginConfiguration.addProperty("caseSensitive","false");
        context.addPluginConfiguration(pluginConfiguration);

        //为生成的Java模型类添加序列化接口，并生成serialVersionUID字段
        PluginConfiguration serialPluginConfiguration = new PluginConfiguration();
        serialPluginConfiguration.setConfigurationType("org.mybatis.generator.plugins.SerializablePlugin");
        serialPluginConfiguration.addProperty("suppressJavaInterface","false");
        context.addPluginConfiguration(serialPluginConfiguration);
...
//        javaModelGeneratorConfiguration.addProperty("style","camelhump");
...
//        tableConfiguration.setColumnRenamingRule(columnRenamingRule);
    ...
    public static void genResponseBo() {
        try {
            ...
            data.put("currentPackage", ProjectConstant.CODE_BASE_PACKAGE);

            File file = new File(PROJECT_PATH + JAVA_PATH + PACKAGE_PATH_COMMON_ENTITY
                    +"ResponseBo.java");
